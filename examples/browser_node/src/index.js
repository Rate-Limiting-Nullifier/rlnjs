...
import { deployERC20, deployRLNContract, deployVerifier, treeDepth, url } from "./configs.js";
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const verifierContract = yield deployVerifier(signer);
        console.log(`Deployed mock verifier at ${yield verifierContract.getAddress()}`);
        const erc20Contract = yield deployERC20(signer, signerTestERC20Amount);
        console.log(`Deployed test ERC20 at ${yield erc20Contract.getAddress()}`);
        const rlnContract = yield deployRLNContract(signer, yield erc20Contract.getAddress(), yield verifierContract.getAddress(), rlnContractArgs.minimalDeposit, rlnContractArgs.treeDepth, rlnContractArgs.feePercentage, rlnContractArgs.feeReceiver, rlnContractArgs.freezePeriod);
        const rlnContractAddress = yield rlnContract.getAddress();
        const rlnContractAtBlock = yield provider.getBlockNumber();
        console.log(`Deployed RLN contract at ${rlnContractAddress} at block ${rlnContractAtBlock}`);
        function createRLNInstance() {
            return __awaiter(this, void 0, void 0, function* () {
                return yield RLN.createWithContractRegistry({
                    /* Required */
                    rlnIdentifier,
                    provider,
        ...
        const proof = yield rln.createProof(epoch, message0);
        if (!(yield rln.verifyProof(epoch, message0, proof))) {
        ...
        try {
            yield rln.createProof(epoch, message1);
        }
        catch (e) {
        ...
        yield mineBlocks(blockHeightFreezePeriodEnds);
        console.log("`releaseWithdrawal`...");
        yield rln.releaseWithdrawal();
        if (yield rln.isRegistered()) {
        ...
        const rlnAnother = yield createRLNInstance();
        ...
        yield rlnAnother.register(messageLimit, new FaultyMessageIDCounter(messageLimit));
        console.log(`Creating proof0 for rlnAnother...`);
        const proof0 = yield rlnAnother.createProof(epoch, message0);
        ...
        const proof1 = yield rlnAnother.createProof(epoch, message1);
        console.log(`rln saving proof0 from rlnAnother...`);
        const res0 = yield rln.saveProof(proof0);
